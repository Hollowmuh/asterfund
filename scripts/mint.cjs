...
async function main() {
    ...
    const usdtTokenAddress = process.env.VITE_USDT_CONTRACT_ADDRESS;
    // const ngnaTokenAddress = process.env.NGNATOKEN_ADDRESS;
    const alchemyApiKey = process.env.INFURA_API_KEY;
    
    // Validate environment variables
    if (!deployerPrivateKey || !usdtTokenAddress || !alchemyApiKey ) {
    ...
    // Mint parameters
    const amount = ethers.parseUnits("1000000000", 18); // Minting 1 million tokens (18 decimals)
    const recipient = '0x8456062624E97F243B0d7601B4D157448e835196';

    try {
        // Call the mint function
        console.log(`Minting ${ethers.formatUnits(amount, 18)} tokens to ${recipient}...`);
        const tx = await USDTToken.mint(recipient, amount);
